...
- 2026-06-21 (Sunday maintenance) — **ToDo.Today GH Actions pipeline (02:30 WITA) is first-mover on well-publicised social-indexed Megatix events.** The curator trigger fires at 03:47 WITA — 75 minutes later — and consistently finds these events already in DB. Confirmed example: Full Moon & Lion's Gate AYMC Aug 8 arrived via the automated harvester Jun 14 (first seen in the Jun 14 log) before the curator independently discovered it Jun 20. Implication: the curator's comparative advantage is NOT re-finding events with heavy Megatix/IG presence — ToDo.Today gets those first. Curator value is in direct-venue walks, quiet community word-of-mouth events, IG-first announcements from smaller venues, and recurring anchors (like weekly kirtans) that aggregators don't surface because they're donation-based / walk-in with no ticket page.
- 2026-06-28 (Sunday maintenance) — **Predict the announcement gap, not just the announcement cluster.** The gap between end-of-June events (Jun 26–29 cluster: Kintsugi, Onanya, Shambala, Walker Ji, ABUELA LUNA) and the first July moon-window events creates a predictable 7–10-day dead zone: organizers haven't yet opened ticket sales for events 3–4 weeks out. Jun 21–28 = 8 consecutive empty/minimal-inbox days. The Jul 1–7 window is when July announcements cluster (new moon ~Jul 14 WITA → event Sat Jul 12–13; July Bhakti Bliss, Levi Banner, Akasha moon series). Similarly expect a gap between Aug 8–14 cluster and Aug new-moon window (~Aug 10–12) and full-moon window (~Aug 22–29). An empty day is not a failure — it is the trough between announcement waves.
- 2026-07-05 (Sunday maintenance) — **Professional enrollment workshops are permanent hard rejects.** Gaya CAC, Ananda Sarita school, Pranaluz immersion, and any event with price >IDR 1,000,000/$50 USD per person + multi-day structured curriculum + certification framing = training program, not community gathering. The price point is the fastest kill signal — if the ticket is over $50 USD per person, check if it's a professional training before reading any further. Do not let a prestigious facilitator name override this: Kim Kindersley, Ananda Sarita, and Pranaluz have all appeared as hard rejects for this reason.
- 2026-07-13 (Sunday maintenance) — **CELESTIA zodiac series rotates venues by edition — never assume.** Confirmed: Gemini 2026 = Nuanu/ASH (GEO-REJECT); the venue is not fixed to Akasha. Any CELESTIA edition (celestia-cancer, celestia-leo, etc.) must have the Megatix slug venue field explicitly confirmed in-zone before ingesting. Do not infer from brand familiarity, prior edition location, or "moved from X to Y" language that describes a single-edition shift. Zodiac-series events from any brand should trigger a venue-check before scoring. This applies to similar rotating-venue formats (SUPERMOON, PURNAMA branding as well).
- 2026-07-19 (Sunday maintenance) — **Named moon ceremonies run on the actual moon date, not the nearest Friday/Saturday.** ABUELA LUNA confirmed Jul 29, 2026 — the full moon itself — not Fri Jul 25 or Sat Jul 26 as predicted across multiple prior watch notes. Raio aligns the ceremony precisely with the moon it's named for; the name is the schedule. Apply moon-date-first assumption to similar ceremonies (ABUELA LUNA, Purnama series, any event whose name or description ties explicitly to a specific moon phase). Only shift to "nearest weekend" logic when the organizer's own communication explicitly names a different date.
- 2026-08-02 (Sunday maintenance) — **August 2026 new moon = August 12 (total solar eclipse), NOT Aug 22–23.** Prior logs (Jul 28–Aug 1) and sources.json (School of Unified Healing entry) incorrectly referenced "~Aug 18" or "Aug 22–23" as the new moon date. Corrected 2026-08-02. Total solar eclipse on Aug 12 makes this a high-ceremony date — expect an announcement cluster ~Aug 8–10 for DTT New Moon Kirtan, Steven Light/SUH Bhakti Kirtan, Levi Banner cacao+dance, and Akasha New Moon Ceremony. Apply moon-date-first: watch for Aug 12 announcements, not nearest-Saturday guesses. Full moon = Aug 28 (partial lunar eclipse). Sources.json School of Unified Healing note corrected.
- 2026-08-09 (Sunday maintenance) — **Day-of-week kill does NOT apply to events with explicit "Special Edition" naming.** MANTRA DANZA "Special ALiVE Ecstatic Dance with SAVABORSA" on Saturday Aug 29 (Radiantly Alive Yoga) was initially dismissed 2026-08-04 as an "aggregator garbled-Saturday artefact" because the regular SAVABORSA series runs Wednesdays. Three-agent multi-source investigation 2026-08-09 confirmed: venue is correct (Jl. Jembawan No. 3, not garbled) and the Saturday is intentional — the "Special ALiVE" branding explicitly marks it as a deliberate deviation from the weekly Wednesday series. The day-of-week kill was designed to catch **stale-year data** (e.g., "Thursday August 31" where Aug 31 2026 = Monday — proof the snippet is from 2023), not intentional special editions. **Discriminator:** if a series has a confirmed day-pattern AND an off-pattern event is explicitly branded "Special Edition," "Special ALiVE," "Special Guest," or given a distinct title that differentiates it from the regular series, treat the deviation as intentional. Only apply the kill to anonymous off-pattern entries with no special branding. Attribution rules are unchanged: AllEvents remains a forbidden aggregator regardless of the event's authenticity — hold until a first-party booking link appears.
- 2026-08-11 (post-mortem) — **Eclipse/new-moon date novelty does not predict announcement volume above normal.** Five tracked Aug 12 total solar eclipse candidates (Steven Light/SUH Bhakti Kirtan, Akasha New Moon with Karunika, MANTRA DANZA biweekly, The Red Tent, Levi Banner cacao+dance) + Pyramids of Chi = zero new inbox events, 9th consecutive empty day. Only DTT New Moon Kirtan was in DB — ingested via aggregator-harvest 8 days prior, not curator discovery. Celestial significance (total solar eclipse, partial lunar eclipse, rare moon phases) does not compress organiser lead times; announcement clusters depend on organiser readiness and ticketing-platform rhythm, not the date's astronomical significance. Apply the same 10–14 day watch window regardless of how notable the moon date is.
- 2026-08-23 (Sunday maintenance) — **AI query-framing hallucination is systemic — strip the suspected date from every verification query.** MANTRA DANZA Aug 29 Special ALiVE produced false-positive "confirmed" results from different search agents on two consecutive days (Aug 22, Aug 23). In both cases, AllEvents ID 200030449185116 was re-verified via a clean neutral query (date stripped from the query text) and returned "29 July 2026" — identical result on both days, different agents. Root cause: agents asked "is Aug 29 confirmed?" generate synthetic confirmation from the query frame, not indexed content. Mitigation (now mandatory for all date-uncertain events): (1) run a second verification with the suspected date removed from the query, (2) demand the exact quoted page title or snippet text, (3) treat any agent reporting "confirmed" without quoted text as hallucination. This rule supersedes the Aug 9 lesson's verification recommendation — apply it universally, not just to MANTRA DANZA.
- 2026-08-23 (Sunday maintenance) — **hoo.be / linktr.ee / bento.me link trees are first-party access paths when Momence and Instagram are egress-blocked.** SAVABORSA's hoo.be/alive.dance was discovered 2026-08-23 via neutral search when Momence and Instagram were both blocked. It is a first-party bio-link aggregator that may expose booking URLs the blocked platforms do not. When a facilitator's primary booking platforms (Momence, IG) are inaccessible in sandbox, search for their hoo.be, linktr.ee, or bento.me page as a fallback. Append any confirmed link trees to the relevant sources.json note immediately.
- 2026-09-06 (Sunday maintenance) — **Yoga Barn Megatix slugs with numeric date suffixes carry reliable date information.** Pattern: `house-of-mantra-sept21`, `beauty-way-july`, `cacao-and-sound-meditation-20july` etc. When a WebSearch snippet returns verbatim "Yoga Barn" venue text alongside a slug containing a month+day suffix, the date is confirmed without additional verification. The day-of-week kill applies only when a snippet contains a *named day* (e.g., "Thursday, August 31st") that contradicts the 2026 calendar — a numeric suffix ("sept21") carries no such contradiction. Yoga Barn hosts kirtan, sound, and bhajan events on any day of the week, not only ecstatic-dance days (Fri/Sun). Do not apply a weekday filter to ceremony/kirtan events at Yoga Barn.
- 2026-09-13 (Sunday maintenance) — **Recurring new moon events at DTT, Akasha, and Yoga Barn ABUELA LUNA have shifted to walk-in format in Q3 2026.** Seven consecutive daily watches (D-7 to D-0) for the Sep 11 new moon cluster produced zero Megatix pages for DTT New Moon Kirtan & Sacred Tea, Akasha New Moon with Karunika, and ABUELA LUNA by Raio. Prior Jul/Aug clusters posted D-7 to D-10. Confirmed pattern shift: these events now run walk-in or announce D-0 via IG story only (not indexable). Do not escalate search frequency for new moon events at these three venues — check once at D-3, accept walk-in if silent, and move on. Full moon editions still ticket per-edition (DTT Full Moon Kirtan slug pattern active; Pyramids of Chi per-edition slugs active). Separately: Jeremy Sol AYMC events confirmed walk-in/first-party-URL in Q3 2026 — the Ganesha Spirit Celebration confirmed via two independent WebSearch queries against the AYMC first-party URL (no Megatix slug ever surfaced). Going forward, confirm AYMC Jeremy Sol events via dual-query against alchemyyogacenter.com directly.
- 2026-09-20 (Sunday maintenance) — **A wrong identifier in `sources.json` is indistinguishable from "this source has no events", and it can null a tier-A source for months.** Blossom Space Ubud has been recorded since 2026-06-27 as reachable via Instagram `@blossom.ubud`. The real handle is **`@blossomspace.ubud`**, confirmed 2026-09-20 by the verbatim indexed profile title "Slow down. Breath in. (@blossomspace.ubud)". Every Blossom walk since June searched a handle that does not exist and correctly returned nothing, and every log recorded that nothing as a null result about the venue rather than about our own record. `blossomubud.com` also does not resolve (ENOTFOUND, Jun 27), so the handle was the only live path. **Rule: when a source returns nothing for three or more consecutive runs, the next check is of our stored identifier, not of the source.** Re-derive the handle/URL from a verbatim indexed title before concluding the venue is quiet. This is the cheapest possible check and it audits the beat rather than re-reading the index.
- 2026-09-20 (Sunday maintenance) — **Write the standing programme of every tier-A/B venue into `sources.json`, or each run pays to re-discover it and then suppress it.** Three priority sources had their actual weekly anchor undocumented: Dragonfly Village (priority_a since May) runs 'Dance of the Dragonfly', Wed 17:00–18:30, Emmalee Bevan / Ritual Beats; Mudra / The Cobra Room runs 'Shakti Bhakti: Kirtan & Cacao', Thu 18:00–19:30, Anu Karoliina & Friends. Both were surfaced fresh this run, recognised as recurring anchors, and suppressed — the same work prior runs did and did not record. An undocumented anchor is a recurring tax: it consumes a search, produces a reject, and teaches the next run nothing. Recording it converts that into a one-time write, and makes the genuinely ingestable case legible — a **dated per-edition slug or an explicitly branded special edition** (2026-08-09 discriminator) against a known anchor.
