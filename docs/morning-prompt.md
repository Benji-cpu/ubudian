...
Also sweep the competitor-harvest aggregators (`sources.json.competitor_harvest`: Blissbase, Soulwise, ToDo.Today, ubud.app, BaliSpirit) for venues + facilitators we don't yet track. Anything that surfaces across ≥2 sources goes straight to `discovered_pending`. **Attribution rules from `curator/playbook.md` apply** — never store aggregator URLs on events, never credit them in descriptions, recreate listings in our voice. They are scouts, not sources. **Use the API-direct paths** documented in `.claude/agents/daily-curator.md` Step 3b — Blissbase exposes a SvelteKit `/__data.json` endpoint that returns events with original-ticket-URL `sourceUrl` field; Soulwise exposes a sitemap of all ~393 listings. ToDo.Today remains Cloudflare-walled.
