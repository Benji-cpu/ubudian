...
export const alt = "Ubudian Guide";
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

const TIER_LABEL: Record<Guide["tier"], string> = {
...
export default async function GuideOgImage({
  ...
  const { data: row } = await supabase
    .from("guides")
    .select("title, subtitle, hero_image_url, card_image_url, tier, intent_tags")
    .eq("slug", slug)
    .eq("status", "published")
    .maybeSingle();

  const guide = (row ?? null) as Pick<
    Guide,
    "title" | "subtitle" | "hero_image_url" | "card_image_url" | "tier" | "intent_tags"
  > | null;
  ...
  const tierLabel = TIER_LABEL[guide.tier];
  const cover = guide.hero_image_url || guide.card_image_url || null;
  const intents = (guide.intent_tags ?? []).slice(0, 3);
