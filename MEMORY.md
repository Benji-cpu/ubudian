...
- ~~The Gemini key 429s at 5 concurrent calls but answers a single call fine: a per-minute rate, not a dead key.~~ **WRONG — measured 2026-09-16 (CRM #331).** Read the 429's violation details, not its message string: `GenerateRequestsPerDayPerProjectPerModel-FreeTier`, quotaValue **20 — twenty requests a day per model**. Pacing to 10/min gave 0 successes in 714s; a single call succeeding is just the tail of a daily budget. Billing on the key's project is the only fix, and the account to enable it on is `b.hemsonstruthers@gmail.com` or `mystechcards@gmail.com` — **never `profbenjo@gmail.com`**, barred from Google Cloud since Aug 2023. AI Studio lists different keys per account, so check the login before concluding a key is missing.
